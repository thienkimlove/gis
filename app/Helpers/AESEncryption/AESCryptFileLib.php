...
class AESCryptFileLib
{
     ...
     * the AES mode is CBC
     * the AES IV value is generated random
     * @param $source_file the file to be encrypted
     * @param $passphrase the combination of password and IV value
     * @param null $dest_file the file after encrypt
     * @param null $ext_data additional data if any
     * @return string the name of encrypted file
     * @throws Exception
     */
    ...
    /**
     * Read the extension of file while encrypting
     * @param $source_file
     * @return array|null
     * @throws Exception
     */
    public function readExtensionBlocks($source_file)
    {
        ...
        if ($source_fh === false) {
            throw new Exception("Cannot open file for reading: " . $source_file);
        }
        ...
        if (bin2hex($version_chunk) === dechex(ord("0"))) {
            ...
            while (true) {
                ...
                if ($ext_length == 0) {
                    ...
                    if ($null_index === false) {
                        throw new Exception("Extension block data at index {$eb_index} has no null splitter byte: " . $source_file);
                    }
            ...
            throw new Exception("Unknown version: " . bin2hex($version_chunk));
        }
    ...
    /**
     * Decrypt file
     * @param $source_file the file to be decrypted
     * @param $passphrase the combination of password and IV value
     * @param null $dest_file the destination for decrypted file
     * @return string the location of decrypted file
     * @throws Exception
     */
    ...
    /** Check out the existent of file and it's readable or not
     * @param $source_file the file to be checked
     * @throws Exception
     */
    private function checkSourceExistsAndReadable($source_file)
    {
        ...
        if (!file_exists($source_file)) {
            throw new Exception($source_file);
        }

        //Source file must be readable
        if (!is_readable($source_file)) {
            throw new Exception("Cannot read: " . $source_file);
        }
    }
    ...
    private function openDestinationFile($source_file, $dest_file, $encrypting = true) {
        ...
        if ($dest_fh === false) {
            throw new Exception("Cannot create for writing:" . $dest_file);
        }
     ...
     * @param $num_bytes
     * @param $chunk_name
     * @param null $unpack_format
     * @param null $expected_value
     * @return array|string
     * @throws Exception
     */
    private function readChunk($source_fh, $num_bytes, $chunk_name, $unpack_format = NULL, $expected_value = NULL)
    {
        ...
        if ($read_data === false) {
            throw new Exception("Could not read chunk " . $chunk_name . " of " . $num_bytes . " bytes");
        }

        if (self::bin_strlen($read_data) != $num_bytes) {
            throw new Exception("Could not read chunk " . $chunk_name . " of " . $num_bytes . " bytes, only found " . self::bin_strlen($read_data) . " bytes");
        }
        ...
        if ($expected_value !== NULL) {
            if ($read_data !== $expected_value) {
                throw new Exception("The chunk " . $chunk_name . " was expected to be " . bin2hex($expected_value) . " but found " . bin2hex($read_data));
            }
        }
        return $read_data;
    }
    ...
    /**
     * Verify the extension data of file while encrypting
     * @param $ext_data
     * @throws Exception
     */
    private function checkExtensionData($ext_data)
    {
        ...
        if (!is_array($ext_data)) {
            throw new Exception("Must be NULL or an array (containing 'extension block' arrays)");
        }
        ...
        foreach ($ext_data as $index => $eb) {
            if (!is_array($eb)) {
                throw new Exception("Extension block at index {$index} must be an array");
            }
            //Each block must contain the array keys 'identifier' and 'contents'
            if (!array_key_exists("identifier", $eb)) {
                throw new Exception("Extension block at index {$index} must contain the key 'identifier'");
            }
            if (!array_key_exists("contents", $eb)) {
                throw new Exception("Extension block at index {$index} must contain the key 'contents'");
            }

            $identifier = $eb['identifier'];
            $contents = $eb['contents'];
            if (!is_string($identifier)) {
                throw new Exception("Extension block at index {$index} has a bad 'identifier' value.  It must be a string.");
            }
            if (!is_string($contents)) {
                throw new Exception("Extension block at index {$index} has a bad 'contents' value.  It must be a string.");
            }

            if (in_array($identifier, $unique_identifiers)) {
                throw new Exception("Extension block at index {$index} contains an 'identifier' which has already been used.  Make sure they are unique.");
            } else {
    ...
    /**
     * Verify the input password is valid or not
     * @param $passphrase
     * @throws Exception
     */
    ...
    {
        if ($passphrase === NULL) {
            throw new Exception("NULL passphrase not allowed");
        }
    }
    ...
    private function doEncryptFile($source_file, $passphrase, $dest_file, $ext_data)
    {
        ...
        if (self::bin_strlen($iv_1) != 16) {
            throw new Exception("Returned an IV which is not 16 bytes long: " . bin2hex($iv_1));
        }
        ...
        if (self::bin_strlen($iv_2) != 16) {
            throw new Exception("Returned an IV which is not 16 bytes long: " . bin2hex($iv_2));
        }
        ...
        if (self::bin_strlen($enc_key_2) != 32) {
            throw new Exception("Returned a random key which is not 32 bytes long: " . bin2hex($enc_key_2));
        }
        ...
        if ($written === false) {
            throw new Exception("Could not write encrypted data to file.  Tried to write " . self::bin_strlen($enc_data) . " bytes");
        }
    ...
    private function validateHMAC($key, $data, $hash, $name)
    {
        ...
        if ($calculated != $actual) {
            ...
            if ($name == "HMAC 1") {
                throw new Exception("{$name} failed to validate integrity of encryption keys.  Incorrect password or file corrupted.");
            } else {
                throw new Exception("{$name} failed to validate integrity of encrypted data.  The file is corrupted and should not be trusted.");
            }
        }
    }
